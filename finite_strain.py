...
def contact_cycle(displ):
	...
	#==============================================================================#
	#BLOCK CALCULATES ITS RESIDUAL AND SEND IT TO THE IDENTER
...
for z in range(int(total_steps)):
	...
	"""
	if app_name == 'IDENTER':
		filefile = open("IDENTER"+str(z+1),'w')
		#for ii in range(num_nodes):
		...
		for bc in boundary_condition_contact:
			for eg in element_groups[physical_names[bc][1]]:
				...
				p2_x = nodes[eg[5]-1][1] + displ[(eg[5]-1)*2]
				p2_y = nodes[eg[5]-1][2] + displ[(eg[5]-1)*2+1]
				filefile.write(str(p1_x))
				filefile.write(" ")
				filefile.write(str(p1_y))
				filefile.write("\n")
				filefile.write(str(p2_x))
				filefile.write(" ")
				filefile.write(str(p2_y))
				filefile.write("\n")
		filefile.close()
	"""
	if app_name == 'BLOCK': 
